		...
		describe( "JWT Security Services", function(){
			...
			given( "an empty jwt token and accessing a secure api call", function(){
				then( "it should block with no authorization", function(){
					...
					expect( event.getCurrentEvent() ).toBe( "api:Home.onInvalidAuth" );
					expect( event.valueExists( "relocate_event" ) ).toBeFalse();
					expect( event.getPrivateValue( "cbsecurity_validatorResults" ).messages ).toInclude( "TokenNotFoundException" );
				});
			});
			...
			given( "an invalid jwt token and accessing a secure api call", function(){
				then( "it should block with no authorization", function(){
					...
					expect( event.getCurrentEvent() ).toBe( "api:Home.onInvalidAuth" );
					expect( event.valueExists( "relocate_event" ) ).toBeFalse();
					expect( event.getPrivateValue( "cbsecurity_validatorResults" ).messages ).toInclude( "TokenInvalidException" );
				});
			});

			given( "an valid jwt token with no required claims and accessing a secure api call", function(){
				then( "it should block with no authorization", function(){
					getRequestContext().setValue( "x-auth-token", "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiJ9.eyJpYXQiOjE1NjkyNzIwOTksInNjb3BlcyI6W10sImlzcyI6Imh0dHA6Ly8xMjcuMC4wLjE6NTY1OTYvIiwic3ViIjoxMjMsImV4cCI6MTU2OTI3MjEwMCwianRpIjoiMTkzQ0NFNUIwNjRENUJEMERENjcxRTQ4N0EzNzI3Q0QifQ.QovEexPi5BCca_N_LHv9R2dUF-GOHbUKOSIzRT7udsHL0zGkkzkVeVPR1_ccxciGoJL_IZaI0AG9_gHKk2Yf9g" );
					var event = execute( route = "/api/secure", renderResults = true );
					expect( event.getCurrentEvent() ).toBe( "api:Home.onInvalidAuth" );
					expect( event.valueExists( "relocate_event" ) ).toBeFalse();
					expect( event.getPrivateValue( "cbsecurity_validatorResults" ).messages ).toInclude( "TokenInvalidException" );
			...
			given( "an valid jwt token that's expired", function(){
				then( "it should block with no authorization", function(){
					...
					expect( event.getCurrentEvent() ).toBe( "api:Home.onInvalidAuth" );
					expect( event.valueExists( "relocate_event" ) ).toBeFalse();
					expect( event.getPrivateValue( "cbsecurity_validatorResults" ).messages ).toInclude( "TokenExpiredException" );
				});
			});

			given( "an valid jwt token but it is not in the storage", function(){
				then( "it should block with no authorization", function(){
					var thisToken = getInstance( "jwtService@cbSecurity" )
						.attempt( "test", "test" );

					getController().getCacheBox().getCache( "default" ).clearAll();
					getRequestContext().setValue( "x-auth-token", thisToken );
					var event = execute( route = "/api/secure", renderResults = true );

					expect( event.getCurrentEvent() ).toBe( "api:Home.onInvalidAuth" );
					expect( event.valueExists( "relocate_event" ) ).toBeFalse();
					expect( event.getPrivateValue( "cbsecurity_validatorResults" ).messages ).toInclude( "TokenRejectionException" );
				});
			});
			...
			given( "an valid jwt token but with an invalid user", function(){
				then( "it should block with no authorization", function(){
					...
					getController().getCacheBox().getCache( "default" )
						.set( "cbjwt_#thisToken.payload.jti#", thisToken );
					var event = execute( route = "/api/secure", renderResults = true );

					expect( event.getCurrentEvent() ).toBe( "api:Home.onInvalidAuth" );
					expect( event.valueExists( "relocate_event" ) ).toBeFalse();
					expect( event.getPrivateValue( "cbsecurity_validatorResults" ).messages )
						.toInclude( "InvalidTokenUser" );
				});
			});
